...
for dep in deps:
    ...
    status = subprocess.run(
        ...
        cwd=os.path.join(script_dir, dep.name),
        capture_output=True,
        text=True
    )
    if status.returncode != 0:
        ...
        exit(1)
    if status.stdout != "" and not os.getenv("CI") and input(f"Warning: {dep.name} has uncommitted changes, do you want to continue? (y/n) ") != "y":
        print("Aborted")
        exit(1)

    # Ensure that there's no manual commits after the tag
    log = subprocess.run(
        ["git", "log", "--oneline", f"{dep.tag}..HEAD"],
        cwd=os.path.join(script_dir, dep.name),
        capture_output=True,
        text=True
    )
    ...
    if log.stdout != "":
        ...
        elif re.search(r"^([a-f0-9]+) Apply Cassia patches", lines[0]) is None:
            # If the commit doesn't match the automatically generated by apply-patches.py
            has_manual_commits = True
        if has_manual_commits and not os.getenv("CI") and input(f"Warning: {dep.name} has manual commits, do you want to continue? (y/n) ") != "y":
            print("Aborted")
            exit(1)

    # Reset to tag
    if subprocess.run(
        ["git", "reset", "--hard", dep.tag],
        stdout=sys.stdout,
        cwd=os.path.join(script_dir, dep.name)
    ).returncode != 0:
